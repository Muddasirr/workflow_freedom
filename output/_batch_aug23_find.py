#!/usr/bin/env python3
"""Aug 23: scrape unsent directory companies for published hiring emails; SMTP verify."""
...
OUT = ROOT / "output" / "emails_go_2026-08-23.csv"
CACHE = ROOT / "outreach" / "mailbox_cache.csv"
NOW = datetime.now(timezone.utc).isoformat(timespec="seconds")

# Prefer product/SWE-friendly regions first
REGION_ORDER = (
    "Remote",
    "Europe",
    "Pakistan",
...
def soft_outlook_ok(email: str, detail: str) -> bool:
    local = email.split("@", 1)[0]
    hiring = any(t in local for t in HIRE_HINTS) or local in {"hello", "contact", "info", "team"}
    text = detail.lower()
    return (
        hiring
        and "catch-all" not in text
        and ("rcpt 250" in text or "recipient ok" in text or "2.1.5" in text)
        and any(x in text for x in ("access denied", "5.7.1", "probe-blocked", "spamhaus", "5.4.1"))
    )
...
def main() -> None:
    ...
    for name, domain, region, city, known in COMPANY_DIRECTORY:
        ...
        if region not in REGION_ORDER:
            continue
        targets.append((name, d, region, known or []))

    # Prefer Remote/Europe/PK first
    targets.sort(key=lambda t: REGION_ORDER.index(t[2]) if t[2] in REGION_ORDER else 99)
    targets = targets[:220]
    ...
    def add(company: str, email: str, domain: str, region: str, source: str) -> None:
        ...
        if local in SKIP_LOCAL:
            return
        if domain in BLOCKED_DOMAINS or domain.endswith(".edu"):
            return
    ...
    def one(item):
        ...
        emails = [e.lower() for e in (known or []) if e] + scrape(domain)
    ...
    for i, row in enumerate(candidates.values(), 1):
        ...
        soft = soft_outlook_ok(email, detail) if not ok else False
        if soft:
            ok = True
            ...
            with CACHE.open("a", encoding="utf-8-sig", newline="") as c:
                csv.writer(c).writerow(
                    [
                        ...
                        email,
                        "strict-valid",
                        "published hiring; real RCPT 250; Outlook probe blocked",
                    ]
                )
    ...
    for r in ready:
        print(f"  {r['HR / Recruiter Email']}  {r['Company']}", flush=True)
